...
INCLUDES := $(INCLUDES) \
	-I $(PDIR)/firmware/fota/fota/include \
...
ESPTOOL = esptool.py --baud 576000 write_flash -u --flash_mode qio \
		  --flash_freq 40m
...
flash_map2:
	...
	$(ESPTOOL) --flash_size 1MB  \
		0x0 	$(BINDIR)/boot_v1.7.bin \
		0x1000  $(BINDIR)/upgrade/user1.1024.new.2.bin \
		0xfc000 $(BINDIR)/esp_init_data_default_v08.bin \
		0xfb000 $(BINDIR)/blank.bin \
		0xfe000 $(BINDIR)/blank.bin
...
flash_map3:
	...
	$(ESPTOOL) --flash_size 2MB  \
		...
		0x1fc000 $(BINDIR)/esp_init_data_default_v08.bin \
		0x1fb000 $(BINDIR)/blank.bin

	#	0x1fe000 $(BINDIR)/blank.bin
...
flash_map4:
	...
	$(ESPTOOL) --flash_size 4MB  \
		...
		0x3fc000 $(BINDIR)/esp_init_data_default_v08.bin \
		0x3fb000 $(BINDIR)/blank.bin
	#	0x3fe000 $(BINDIR)/blank.bin 
...
flash_map5:
	...
	$(ESPTOOL) --flash_size 16m-c1  \
		0x0 	$(BINDIR)/boot_v1.2.bin \
		0x1000  $(BINDIR)/upgrade/user1.2048.new.5.bin \
		0x1fc000 $(BINDIR)/esp_init_data_default_v05.bin \
		0x1fb000 $(BINDIR)/blank.bin \
		0x1fe000 $(BINDIR)/blank.bin 
